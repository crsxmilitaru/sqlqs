...
function buildRenameSql(
  ...
  const oldQualified = escape(`${quoteIdentifier(schema)}.${quoteIdentifier(oldName)}`);
...
export default function RenameDialog(props: Props) {
  ...
  const validation = createMemo<string | null>(() => {
    ...
    if (value === props.name) return "New name must differ from the current name.";
    if (value.length > MAX_NAME_LEN) return `Name cannot exceed ${MAX_NAME_LEN} characters.`;
    if (INVALID_RE.test(value)) return "Name cannot contain quotes or square brackets.";
    return null;
  });
  ...
  const handleConfirm = async () => {
    ...
    try {
      const sql = buildRenameSql(props.objectType, props.database, props.schema, props.name, trimmed);
      ...
      const message = String(err ?? "Rename failed").replace(/^Error:\s*/i, "").replace(/^Query failed:\s*/i, "");
      setError(message);
      setExecuting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          <div class="flex items-center justify-between px-6 py-4 border-b border-overlay-xs">
            <div class="flex items-center gap-3 min-w-0">
              ...
              <div class="flex flex-col min-w-0">
                <h2 class="text-m font-semibold text-text">Rename {objectTypeLabel(props.objectType)}</h2>
          ...
          <div class="px-6 py-4 flex flex-col gap-3">
            ...
            <div class="flex items-start gap-2 p-3 rounded-lg bg-warning/5 border border-warning/20 text-xs text-warning/90">
              ...
              <span>
                <code>sp_rename</code> only updates the object's name. Other objects that reference it by name (procedures, views, triggers, etc.) will break until you ALTER them.
              </span>
            </div>
          </div>
          ...
          <div class="flex justify-end gap-3 px-6 py-4 border-t border-border rounded-b-2xl">
            ...
            <button
              ...
              class={`btn px-6 py-1.5 gap-2 transition-all ${
                success() ? "bg-success border-success text-white" : "btn-primary"
              }`}
            >
              <Show when={success()}>
