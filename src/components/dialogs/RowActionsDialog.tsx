...
function isDateTimeType(typeName: string): boolean {
  return ["datetime", "datetime2", "smalldatetime"].includes(
...
function isLongTextType(typeName: string): boolean {
  ...
  return (
    normalized.includes("(max)") || ["text", "ntext", "xml"].includes(base)
  );
}
...
export default function RowActionsDialog(props: Props) {
  ...
  const [primaryKeyCols, setPrimaryKeyCols] = createSignal<Set<string>>(
    new Set(),
  );
  ...
  createEffect(() => {
    ...
    setValues(
      props.row.map((cell, index) => toEditorValue(cell, columns[index]!)),
    );
    setHydrated(true);
  });
  ...
  const missingPrimaryKeyColumns = createMemo(() => {
    const available = new Set(
      mergedColumns().map((column) => column.name.toLowerCase()),
    );
                  ...
                  >
                    ...
                    <div class="flex-1 flex items-start gap-1.5 min-w-0">
                          ...
                          <div class="flex-1 px-2.5 py-2 rounded-md bg-surface/40 border border-border/20 text-sm font-mono select-text min-h-[36px] flex items-center overflow-hidden whitespace-pre-wrap break-all">
                            {inputKind() === "binary" ? (
                              <span class="text-text/80">
                                {formatBinaryValue(cellValue())}
                              </span>
                            ) : cellValue() != null ? (
                              <span class="truncate text-text/80">
                                {String(cellValue())}
                              </span>
                            ) : (
                      ...
                      >
                        <Show
                          when={
                            cellValue() !== null || inputKind() === "checkbox"
                          }
                          fallback={
