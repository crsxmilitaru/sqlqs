...
function cleanError(err: unknown, fallback: string): string {
  return String(err ?? fallback)
    .replace(/^Error:\s*/i, "")
    .replace(
      /^(Backup|Restore|Schedule creation|Schedule deletion) failed:\s*/i,
      "",
    );
}
...
function makeRestorePath(
  defaults: BackupDefaults | null,
  targetDb: string,
  file: BackupFileInfo,
  index: number,
): string {
  ...
  const folder =
    (isLog ? defaults?.log_directory : defaults?.data_directory) || "";
...
export default function BackupRestoreDialog(props: Props) {
  const preferredDatabase = () =>
    props.initialDatabase || props.currentDatabase || props.databases[0] || "";
  ...
  const [scheduleFrequency, setScheduleFrequency] =
    createSignal<BackupScheduleFrequency>("daily");
  ...
  async function inspectBackup() {
    ...
    try {
      ...
      const firstDataFile = files.find(
        (file) => file.file_type.toUpperCase() !== "L",
      );
      const nextTargetDatabase =
        targetDatabase() ||
        firstDataFile?.logical_name.replace(/_data$/i, "") ||
        "RestoredDatabase";
      setTargetDatabase(nextTargetDatabase);
      setFileMoves(
        files.map((file, index) => ({
          logical_name: file.logical_name,
          physical_name: makeRestorePath(
            defaults(),
            nextTargetDatabase,
            file,
            index,
      ...
      setSuccess(
        `Loaded ${files.length} file${files.length === 1 ? "" : "s"} from backup.`,
      );
    } catch (err) {
  ...
  function refreshRestoreMoves() {
    setFileMoves(
      backupFiles().map((file, index) => ({
        ...
        physical_name: makeRestorePath(
          defaults(),
          targetDatabase() || "RestoredDatabase",
          file,
          index,
        ),
      })),
  ...
  async function saveSchedule() {
    ...
    setScheduleError(null);
    try {
      const result = await invoke<BackupOperationResult>(
  ...
  async function deleteSchedule(jobName: string) {
    ...
    setScheduleError(null);
    try {
      await invoke<BackupOperationResult>("delete_backup_schedule", {
        jobName,
      });
  ...
  function updateMove(index: number, physicalName: string) {
    setFileMoves((prev) =>
      prev.map((move, i) =>
        i === index ? { ...move, physical_name: physicalName } : move,
      ),
    );
  }
  ...
  const renderCheck = (
    ...
    <label
      class={`flex items-center gap-2 text-s text-text-muted ${disabled ? "opacity-50" : "cursor-pointer"}`}
    >
      <input
        type="checkbox"
        name={name}
    ...
    >
          ...
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="flex flex-col min-w-0">
              ...
              <p class="text-xs text-text-muted truncate">
                Database backup and restore wizard with SQL Server Agent
                scheduling
              </p>
            </div>
          </div>
            ...
            <div class="grid grid-cols-[1fr_220px] gap-5 max-md:grid-cols-1">
                ...
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <Dropdown
                      ...
                      onChange={(value) =>
                        setScheduleFrequency(value as BackupScheduleFrequency)
                      }
                    />
                  </div>
                  <div>
                ...
                <Show when={scheduleFrequency() === "monthly"}>
                  <div class="max-w-[160px]">
                    ...
                    <Input
                      ...
                      value={monthlyDay()}
                      onInput={(e) =>
                        setMonthlyDay(Number(e.currentTarget.value) || 1)
                      }
                    />
                  </div>
                </Show>
              ...
              <div class="flex flex-col gap-3 text-sm">
                <div class="rounded-lg border border-border/50 bg-surface-overlay/30 p-3">
                  <div class="text-xs uppercase tracking-wider text-text-muted mb-2">
                    Options
                  </div>
                ...
                <div class="rounded-lg border border-border/50 bg-surface-overlay/30 p-3 text-s text-text-muted leading-relaxed">
                  SQL Server writes to paths visible from the database server,
                  not necessarily this workstation.
                </div>
              </div>
            </div>
          </Show>
              ...
              <div class="grid grid-cols-[1fr_auto] gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onInput={(e) =>
                      handleSourcePathChange(e.currentTarget.value)
                    }
                  />
                </div>
                ...
                <Show
                  ...
                  fallback={
                    <div class="px-3 py-8 text-center text-sm text-text-muted">
                      Inspect a backup file to load logical files.
                    </div>
                  }
                >
                  <For each={fileMoves()}>
                    {(move, index) => {
                      ...
                      return (
                        <div class="grid grid-cols-[160px_96px_1fr] gap-3 px-3 py-2 border-t border-border/30 items-center">
                          <div class="min-w-0">
                            <div
                              class="text-sm text-text font-mono truncate"
                              title={move.logical_name}
                            >
                              {move.logical_name}
                            </div>
                            <div class="text-[10px] text-text-muted">
                              {file()?.file_type === "L" ? "Log" : "Data"}
                            </div>
                          </div>
          ...
          <Show when={activeTab() === "schedules"}>
            <div class="flex flex-col gap-3">
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p class="text-xs text-text-muted">
                    Schedules are stored in SQL Server Agent.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <Show
                ...
                fallback={
                  <div class="px-3 py-10 text-center text-sm text-text-muted border border-border/50 rounded-lg">
                    No SQLQS backup schedules found.
                  </div>
                }
              >
                <div class="rounded-lg border border-border/50 overflow-hidden">
                      ...
                      <div class="grid grid-cols-[1fr_170px_auto] gap-3 px-3 py-3 border-b last:border-b-0 border-border/30 items-center">
                        <div class="min-w-0">
                          ...
                          <div class="text-xs text-text-muted truncate">
                            {schedule.schedule_name || "No schedule attached"}
                          </div>
                        </div>
        ...
        <div class="flex items-center justify-between gap-3 px-6 py-4 border-t border-border rounded-b-2xl">
          <div class="text-xs text-text-muted/70">
            <Show when={activeTab() === "backup"}>
              {backupTypeLabel(backupType())} backup for{" "}
          ...
          <div class="flex gap-3">
            ...
            <Show when={activeTab() === "backup"}>
              <button
                ...
                disabled={
                  busy() || !backupDatabase() || !scheduleFolder().trim()
                }
                class="btn btn-secondary px-4"
              >
                <Show
              ...
              <button
                ...
                onClick={runBackup}
                disabled={
                  busy() || !backupDatabase() || !destinationPath().trim()
                }
            ...
            <Show when={activeTab() === "restore"}>
              <button
                ...
                onClick={runRestore}
                disabled={
                  busy() || !sourcePath().trim() || !targetDatabase().trim()
                }
                class="btn btn-primary px-5"
              >
                <Show
