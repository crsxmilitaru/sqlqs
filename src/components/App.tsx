...
export default function App() {
  ...
  const [isConnectionDialogOpen, setIsConnectionDialogOpen] = createSignal(false);
  ...
  // Set platform class on document root
  onMount(() => {
  ...
  // Register SQL file handlers (startup path + event listener)
  createEffect(() => {
    let isMounted = true;
  ...
  // Global keyboard shortcuts (open file, settings)
  createEffect(() => {
  ...
  // Close object jump when dialogs open or connection lost
  createEffect(() => {
    if (!canOpenObjectJump() || hasBlockingDialog()) {
  ...
  // Start background object indexing after connection
  createEffect(() => {
    ...
    const startIndexingInBackground = async () => {
      try {
        const { invoke } = await import("@tauri-apps/api/core");
        const newStatus = await invoke<ServerObjectIndexStatus>("start_server_object_indexing");
        ...
        {isSettingsOpen() ? (
          <SettingsView
            ...
            renderLayout={(sidebar, content) => (
              <>
                <div style={{ width: `${explorerWidth()}px` }} class="app-sidebar-surface flex-shrink-0 overflow-hidden relative flex flex-col z-10 animate-in fade-in">
                  {sidebar}
                </div>
                <div class="resizer resizer-h" onMouseDown={handleExplorerResize} />
                <main class={`flex-1 flex flex-col overflow-hidden bg-surface-panel rounded-tl-2xl border-t border-l-0 border-[color-mix(in_srgb,var(--color-border)_50%,transparent)] relative`}>
          ...
          <>
            {connected() && isSidebarOpen() && (
              <>
                <div style={{ width: `${explorerWidth()}px` }} class="app-sidebar-surface flex-shrink-0 overflow-hidden relative z-10">
                  <ObjectExplorer
                    databases={databases()}
            ...
            <main class={`flex-1 flex flex-col overflow-hidden bg-surface-panel ${isSidebarOpen() && connected() ? 'rounded-tl-2xl border-t border-l-0' : 'rounded-none border-l border-t'} border-[color-mix(in_srgb,var(--color-border)_50%,transparent)] relative transition-colors duration-[var(--duration-slow)]`}>
              <QueryEditorPanel
                tabs={tabs()}
                activeTabId={activeTabId()}
                onTabAdd={addTab}
