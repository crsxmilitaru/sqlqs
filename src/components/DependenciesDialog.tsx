...
export default function DependenciesDialog(props: Props) {
  ...
  const formatRow = (row: DependencyRow): string => {
    const parts = [row.database, row.schema, row.name].filter((p) => p && p.length > 0);
    return parts.map((p) => `[${p}]`).join(".");
  };

  const tabCount = (tab: Tab) => (tab === "used-by" ? usedBy().length : references().length);
  const tabLoading = (tab: Tab) => (tab === "used-by" ? usedByLoading() : referencesLoading());
          ...
          <For each={rows}>
            {(row) => (
              <div class="flex items-center gap-3 px-4 py-2.5 hover:bg-surface-overlay/30">
                <i class={`fa-solid ${classIcon(row.class)} text-text-muted/60 text-xs w-4 text-center`} />
    ...
    >
      ...
      >
        <div class="flex items-center justify-between px-6 py-4 border-b border-overlay-xs">
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="flex flex-col min-w-0">
              <h2 class="text-m font-semibold text-text">{objectTypeLabel(props.objectType)} Dependencies</h2>
        ...
        <div class="flex border-b border-border/30 px-2">
          <For each={[
            { id: "used-by" as const, label: "Used By", hint: "What references this object" },
            { id: "references" as const, label: "References", hint: "What this object references" },
          ]}>
            {(tab) => (
              <Tooltip content={tab.hint} placement="bottom">
                ...
                >
                  ...
                  <Show when={!tabLoading(tab.id)}>
                    <span class={`text-[10px] rounded-full px-1.5 py-0.5 min-w-[18px] text-center ${
                      activeTab() === tab.id ? "bg-accent/20 text-accent" : "bg-surface-overlay text-text-muted"
                    }`}>
                      {tabCount(tab.id)}
                    </span>
                  </Show>
