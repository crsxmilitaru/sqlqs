...
export default function ToolsPopup(props: Props) {
  ...
  return (
    <div
      ref={popupRef}
      class="absolute bottom-full mb-1.5 left-0 w-[240px] bg-surface-panel border border-border/50 rounded-lg animate-popover-in p-1 shadow-md shadow-black/20"
    >
        ...
        <For each={AI_TOOLS}>
          {(tool) => {
            ...
            return (
              ...
              >
                <i
                  class={`${tool.icon} text-s w-4 text-center ${isOn() ? "text-accent" : "text-text-muted opacity-80"}`}
                />
                <div class="flex-1 min-w-0 pr-2 flex flex-col gap-0.5">
                  <div
                    class={`text-xs font-medium truncate ${isOn() ? "text-text" : "text-text/80"}`}
                  >
                    {tool.label}
                  </div>
                  <div class="text-[10px] text-text-muted leading-tight truncate">
                    {tool.description}
                  </div>
                </div>
                <div
